<script lang="ts">
  /** Event — что произошло.
   *  Канал кодирования: ВРЕМЕННОЙ ЭФФЕКТ (вспышка, появление, волна, анимация).
   *  Переход (арность Δ) — отмечает смену состояния Entity между T0 и T1. */
  let {
    label = '',
    variant = 'flash',
  }: {
    label?: string;
    variant?: 'flash' | 'appear' | 'wave';
  } = $props();
</script>
