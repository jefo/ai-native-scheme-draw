...
- **Canvases — semantic DSL kits** (`src/lib/canvases/`) — NOT a UI kit of Card/Box/Row wrappers. Each canvas is a *grammar of thought* with its own primitives (Positioning Map: Axis/Player/Territory/Vector/Tension; User Story: Actor/Action/Artifact/System/Handoff/Decision/Outcome — повтор Artifact = та же entity, канвас связывает вхождения нитью через реестр имён; Roadmap: Horizon/Phase/Milestone/Dependency — три зоны-панели Now/Next/Later в регистре кита (панель + чипы, как Lean): фазы — нейтральные карточки, вехи — mint-чипы с ромбом (единственный семантический цвет — достижение), зависимости — пунктирные дуги по имени вехи). Target layer beneath: a shared **Visual Semantics** vocabulary (Actor, Entity, Action, Artifact, Constraint, Decision, Outcome, Relationship, Tension, Boundary, Direction). Components carry meaning: `<Player name="Us" x={82} y={80} state="focus" />` — and the canvas resolves geometry: players register by name via Svelte context (`positioning-context.ts`), so `<Tension between={["Us","Miro"]} />` works with zero coordinates. Story's artifacts do the same (`story-context.ts`): `<Artifact name="Brief" />` twice in a Story = the same entity, drawn as a dashed thread. Roadmap's milestones likewise (`roadmap-context.ts`): `<Dependency from="Canon" to="Render" />` draws an arc between the two diamonds by name. Dual API: data props OR slots (children = the DSL). Visual register: **technical editorial** — thin geometry, typographic hierarchy, accents only on semantic state. The canonical canvas register — `--canvas-panel-bg` panels + glass chips (sans 13px/600 content, mono 11px/600 tags & labels, mono 10px uppercase legends, radius 8, colored borders with glow) — is the house style ALL DSL kits share: POM players are chips with state-colored borders (focus amber glow-dot, competitor rose, emerging cyan), story steps are chips (actor cyan dot + ink name, artifact sticky-blue glass, decision amber diamond-chip, outcome mint ✓, system = dashed cyan band); the roadmap's mint milestone chips are the one semantic accent in its grammar. Formula: *HTML describes documents. React describes interfaces. This DSL describes reasoning.*
- **Composition system** (`src/lib/canvases/stage/`, `layout/`) — the kit's media-composition layer: **Stage → Composition → Organism → Primitive** (a declarative visual composition system, not a diagram renderer). `Stage` is the media container (aspect 16:9/9:16, density compact|editorial|cinematic, safeArea shorts, composition derived from aspect) and provides density-adaptive scales as CSS custom properties (`--stage-gap-*`, `--stage-type-*`, `--stage-safe-*`). Layout primitives carry the semantic placement vocabulary — `<Stack gap="xl">`, `<Split ratio="2:1">`, `<Zone placement="safe-top">`, `<Rail side="right">` — consumers never see grid coordinates. Two levels stay separate: layout primitives (Stack/Cluster/Split/Grid/Rail/Stage/Zone/Frame/Overlay) vs semantic organisms (PositioningMap/Player/Axis/Tension, …). **Composition variants replace responsive design** (organisms switch layout via `composition`, never media queries — this is media composition, not a webpage). No diagram libraries (React Flow/ELK/D3/Cytoscape): the problem is a known composition, not unknown-graph layout; SVG is reserved for decorative/geometric primitives (Tension line, arrowheads, later Bracket/Connector/AxisMarker). The LLM model: the model emits semantic composition (`<Scene><Hero/><Comparison/><Tension/><Insight/></Scene>`), the design system compiles it to visuals.
